...
router.post('/roleplay-character', async (request, response) => {
  try {
    ...
    const addressMatch = !nftOwnershipMatch && !altNftOwnershipMatch && !nftAddressMatch && !txHistoryMatch && !altTxHistoryMatch ? prompt.match(transactionHistoryQueryPatterns.addressPattern) : null;

    // Finally check for gas price queries
    const gasMatch = !nftOwnershipMatch && !altNftOwnershipMatch && !nftAddressMatch && !txHistoryMatch && !altTxHistoryMatch && !addressMatch ? prompt.match(gasQueryPatterns.primary) : null;
    const altGasMatch = !nftOwnershipMatch && !altNftOwnershipMatch && !nftAddressMatch && !txHistoryMatch && !altTxHistoryMatch && !addressMatch && !gasMatch ? prompt.match(gasQueryPatterns.alternative) : null;

    // Check for token price queries
    const tokenPriceMatch = !nftOwnershipMatch && !altNftOwnershipMatch && !nftAddressMatch && 
                            !txHistoryMatch && !altTxHistoryMatch && !addressMatch && 
                            !gasMatch && !altGasMatch ? prompt.match(tokenPriceQueryPatterns.primary) : null;
                            
    const altTokenPriceMatch = !nftOwnershipMatch && !altNftOwnershipMatch && !nftAddressMatch && 
                              !txHistoryMatch && !altTxHistoryMatch && !addressMatch && 
                              !gasMatch && !altGasMatch && !tokenPriceMatch ? 
    ...
    else if (txHistoryMatch || altTxHistoryMatch || addressMatch) {
      let address, network;
